...
if query:
    query_vector = embed_query(query) #([query])[0]
    ...
    results = search(
        ...
        filename=None if selected_doc == "All PDFs" else selected_doc  ## if all selected, then passing None will iterate through all pdf by default
    )

    st.write(f"📄 {results[0]['doc_id']} (score={results[0]['score']:.3f})")
    st.write(f"- {results[0]['text']}")